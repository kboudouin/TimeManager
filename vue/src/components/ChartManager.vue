...
const resp = await axios.get(`http://54.88.141.45:4000/api/workingtimes/${id}?start=${dateFilter.value.start}T00:00:00Z&end=${dateFilter.value.end}T00:00:00Z`);
  ...
  if(resp.data && resp.data.workingtimes) {
    ...
    resp.data.workingtimes.forEach(wt => {
      ...
      totalWorkedHours.value = Object.values(workByDay).reduce((a, b) => a + b, 0);
    ...
    dailyData.value = {
      ...
      datasets: [{ label: 'Daily Working Hours', data: Object.values(workByDay), backgroundColor: '#ed574c' }]
    };

    // Prepare the weekly data
    weeklyData.value = {
      labels: Object.keys(workByWeek),
      datasets: [{ label: 'Weekly Working Hours', data: Object.values(workByWeek), backgroundColor: '#4caf50' }]
    };

    // Prepare the comulative data

    Object.keys(workByDay).sort().forEach(day => {
...
cumulativeData.value = {
  ...
  datasets: [{ label: 'Cumulative Work Hours', data: workByCumulative.map(d => d.cumulativeWorkHours), borderColor: '#ffcc00' }]
};
  }
  loading.value = false;
};
...
<template>
    ...
    <!-- Date Filter + Stats -->
   <div class="flex items-center p-4 bg-white rounded-lg scale-95 mb-6">
      <span class="mr-4 text-black font-bold">Filter by Date:</span>
      <input type="date" v-model="dateFilter.start" class="p-2 rounded font-bold">
  ...
  <div v-if="loading">
    ...
    <div role="status">
        <svg aria-hidden="true" class=" inline w-32 h-32 mr-42 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <div v-else>
    <div style="display: flex; justify-content: space-around; align-items: center;">
      <!-- Daily Chart -->
      <div class="m-4 bg-white p-4 rounded-lg" v-if="dailyData" style="flex: 1;">
        <Bar id="Daily-Bar-Chart" :options="chartOptions" :data="dailyData" />
      </div>

      <!-- Weekly Chart -->
      <div class="m-4 bg-white p-4 rounded-lg" v-if="weeklyData" style="flex: 1;">
        <bar id="Weekly-Bar-Chart" :options="chartOptions" :data="weeklyData" />
      </div>
    </div>

    <!-- Cummulative Chart -->
    <div class="m-4 bg-white p-4 rounded-lg" v-if="cumulativeData" style="flex: 1;">
      <Line id="Cumulative-Line-Chart" :options="chartOptions" :data="cumulativeData" />
    </div>
