...
<template>
  ...
  <div class="stat">
    ...
    <input
      ...
      v-model="newusername"
    />
    <button @click="updateusername" class="btn btn-active">UPDATE USERNAME</button>
  </div>
  ...
  <div class="stat">
    ...
    <input
      ...
      v-model="newmail"
    />
    <button @click="updatemail" class="btn btn-active">UPDATE EMAIL</button>
  </div>
    ...
    <div class="card flex-shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
      <form class="card-body">
        ...
        <div class="form-control">
          ...
          <input type="" placeholder="username" class="input input-bordered" required />
        </div>
          <label class="label">
            <span class="label-text">Email</span>
          </label>
          <input type="email" placeholder="email@mail.com" class="input input-bordered" required />
        </div>
...
export default {
  ...
  Deleteacc(){
    ...
    axios.delete(`http://localhost:4000/api/users/`+localStorage.getItem("userId"))
    .then(response => {
  ...
  updatemail(){
    ...
    const userId = localStorage.getItem("userId");
    axios.put("http://localhost:4000/api/users/"+userId+"?email="+this.newmail)
    .then(response => {
  ...
  updateid(){
    ...
    axios.put("http://localhost:4000/api/users/"+userId+"?id="+this.newid)
    .then(response => {
  ...
  updateusername(){
    ...
    axios.put("http://localhost:4000/api/users/"+userId+"?username="+this.newusername)
    .then(response => {
