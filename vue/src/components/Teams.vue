<template>
  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg z-10 mt-12">
    <carousel>
      <carousel-item v-for="group in chunkedTeams" :key="group[0].id">
...
import { Carousel, CarouselItem } from "daisyui";
...
const chunkedTeams = ref([]);

watch(teams, () => {
  chunkedTeams.value = chunkArray(teams.value, 3);
});
